    ...
    )
{
    ...
    INFO("         L_IS = %s GPIO_%d ",Connector::uextPin(LP_SCL),uext.toPin(LP_SCL));
    INFO("         R_IS = %s GPIO_%d ",Connector::uextPin(LP_MISO),uext.toPin(LP_MISO));
    INFO("         L_EN = %s GPIO_%d ",Connector::uextPin(LP_MOSI),uext.toPin(LP_MOSI));
    INFO("         R_EN = %s GPIO_%d ",Connector::uextPin(LP_CS),uext.toPin(LP_CS));
    INFO("        L_PWM = %s GPIO_%d ",Connector::uextPin(LP_TXD),uext.toPin(LP_TXD));
    INFO("        R_PWM = %s GPIO_%d ",Connector::uextPin(LP_SCK),uext.toPin(LP_SCK));
    INFO(" Tacho Chan A = %s GPIO_%d ",Connector::uextPin(LP_RXD),uext.toPin(LP_RXD));
    INFO(" Tacho Chan B = %s GPIO_%d ",Connector::uextPin(LP_SDA),uext.toPin(LP_SDA));
}
...
/**
 * @brief motor moves in backward direction, with duty cycle = duty %MESSAGE: Entering directory `/home/lieven/workspace/vertx-esp32'
/bin/sh -c 'make'
...
void MotorSpeed::run()
{
    ...
    while(true) {
        ...
        _rpmMeasured= _direction*_rpmMeasured; // introduce small non-zero
